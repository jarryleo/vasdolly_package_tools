...
class CButton extends StatefulWidget {
  const CButton({
    ...
    this.child,
    this.type = CButtonType.solid,
    this.shape = CButtonShape.standard,
    this.color = QColors.mainColor,
    this.textColor,
    this.size = 50,
    this.borderSide,
    this.text,
  ...
  /// Button type of [CButtonType] i.e, solid, outline, outline2x, transparent
  final CButtonType type;

  /// Button type of [CButtonShape] i.e, standard, pills, square, shadow, icons
  final CButtonShape shape;
...
class _CButtonState extends State<CButton> {
  ...
  Widget? icon;
  Function? onPressed;
  late CButtonType type;
  late CButtonShape shape;
  late double size;
  late CPosition position;
  ...
  @override
  void didUpdateWidget(CButton oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    Color getDisabledFillColor() {
      if (widget.type == CButtonType.transparent ||
          widget.type == CButtonType.outline ||
          widget.type == CButtonType.outline2x) {
    ...
    Color getDisabledTextColor() {
      ...
      } else if (widget.type == CButtonType.outline ||
          widget.type == CButtonType.outline2x ||
          widget.type == CButtonType.transparent) {
        return color;
      } else {
        return QColors.mainText;
      }
    }

    Color getTextColor() {
      if (widget.type == CButtonType.outline ||
          widget.type == CButtonType.outline2x ||
          widget.type == CButtonType.transparent) {
    ...
    final BorderSide outlineBorder = BorderSide(
      ...
      width: (widget.borderSide?.width ??
          (widget.type == CButtonType.outline2x ? 2.0 : 1.0)),
    );
    ...
    final BorderSide shapeBorder = widget.type == CButtonType.outline ||
            widget.type == CButtonType.outline2x
        ? outlineBorder
        : widget.borderSide ??
            BorderSide(
    ...
    } else if (shape == CButtonShape.square) {
      shapeBorderType = RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(50),
        side: shapeBorder,
      );
    } else if (shape == CButtonShape.standard) {
      shapeBorderType = RoundedRectangleBorder(
      ...
      if (widget.type != CButtonType.transparent) {
        if (widget.boxShadow == null && widget.buttonBoxShadow != true) {
          ...
          return BoxDecoration(
            color: widget.type == CButtonType.transparent ||
                    widget.type == CButtonType.outline ||
                    widget.type == CButtonType.outline2x
                ? Colors.transparent
                : color,
            borderRadius: widget.shape == CButtonShape.pills
                ? BorderRadius.circular(50)
                : widget.shape == CButtonShape.standard
    ...
    final Widget result = Container(
      ...
      decoration: widget.type == CButtonType.solid ? getBoxShadow() : null,
      child: Material(
        ...
        shape: widget.type == CButtonType.transparent
            ? null
        ...
        child: InkWell(
          ...
          customBorder: widget.type == CButtonType.transparent
              ? null
              : widget.borderShape ?? shapeBorderType,
